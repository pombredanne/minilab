...
from ia.ocr.OpenANPR.anpr.preprocess import *
from ia.ocr.OpenANPR.anpr.contour import *
...
def extract_plate(image, cluster):
	...
	getattr(cv,'GetRotationMatrix2D')((cx,cy), angle*180.0/math.pi, 1, matrix)
	warp = cv.CreateImage(cv.GetSize(image),
		cv.IPL_DEPTH_8U, 3)
	cv.WarpAffine(image, warp, matrix)
	ret = cv.CreateImage((int(h + cluster[0].dia * 3.0)+1, int(cluster[0].dia * 1.5)+1), cv.IPL_DEPTH_8U, 3)
